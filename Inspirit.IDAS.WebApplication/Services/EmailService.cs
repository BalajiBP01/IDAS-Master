...
namespace Inspirit.IDAS.WebApplication
{
    public class EmailService
    {
        ...
        public async Task<string> SendEmailAsync(string To, string subject, string msgBody, string attachmentFilePath = "", string reply = "", BodyBuilder BodyBuilder = null, bool iscontactus = false, string AttachmentName = "")
        {
            string Message = "";
            ...
            try
            {
                ...
                if (iscontactus == false)
                    ...
                    Email = _dbContext.ApplicationSetting.First(t => t.SettingName == "Contact Us Email Id").SettingValue;
                ...
                if (m.Count > 0)
                {
                    ...
                    foreach (var con in m)
                    {
                        ...
                        {
                            string msg = getImage(con.ToString(), i);
                            ...
                            emailMessage.Body = builder.HtmlBody.Replace(con.ToString(), "<img  style=\"width:100%;height:auto\" src=\"" + imgpath + "\"/>");
        ...
        public string SendEmail(string To, string subject, string msgBody)//, string attachmentFilePath = "", string reply = "", BodyBuilder BodyBuilder = null, bool iscontactus = false, string AttachmentName = "")
        {
            ...
            try
            {
                ...
                //else
                //{
                //    emailMessage.Body = msgBody;//builder.HtmlBody;
                //}
                using (SmtpClient client = new SmtpClient())
                {
                    SmtpClient smtp = new SmtpClient();
        ...
        public string getImage(string htmlcontent, int i)
        {
            string message = string.Empty;
            ...
            string folderName = "ImageForMail";
            string webRootPath = _hostingEnvironment.WebRootPath;
            string newPath = Path.Combine(webRootPath, folderName);
            if (!Directory.Exists(newPath))
                Directory.CreateDirectory(newPath);
