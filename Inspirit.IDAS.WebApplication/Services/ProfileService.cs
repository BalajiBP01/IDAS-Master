...
namespace Inspirit.IDAS.WebApplication
{
    public class ProfileService
    {
            ...
            try
            {
                if (request.SpouseID != null)
                {
                    ...
                    using (con)
                    {
                        ...
                        profile = GetProfileData(lookupLst, request.istrailuser, data, con, request);
                        CustomerLogData logs = new CustomerLogData();

                        if (request.istrailuser == false)
                        {
                            // krishna pending isXDS
                            var reslogs = _CustomerLogService.UpdateCustomerLog(request.userId, request.customerId, "Tracing", "Profile", request.SearchCriteria, profile.idNumber, request.InputType, request.istrailuser,null,null,false,null);

                            Customer cust = _IDAScontext.Customers.Where(c => c.Id == request.customerId).FirstOrDefault();
                            CustomerUser user = _IDAScontext.CustomerUsers.Where(c => c.Id == request.userId).FirstOrDefault();
                            profile.CompanyName = cust.RegistrationName;
                            profile.RefNum = reslogs.Result.RefNum.ToString();
                            profile.SearchCriteria = reslogs.Result.SearchCriteria;
                            // issue code
                            profile.CustomerRefNum = reslogs.Result.CustomerRefNum;
                            /* Customer log is the ideal place
                            if (reslogs.Result.CreditPoints == 1 && cust.IsXDS == true)
                            {
                                // xds api
                                ...
                                string XDSUsername = user.Email;
                                string firstName = null;
                                string surName = null;
                                string BirthDate = null;
                                string InspiritRefNum = reslogs.Result.RefNum.ToString();
                                string EnquiryInput = reslogs.Result.SearchCriteria;
                                // issue code pending as it may not have value
                                string XDSRefNum = reslogs.Result.CustomerRefNum.ToString();
                                string XDSVoucherCode = reslogs.Result.CustomerRefNum.ToString();
                                // issue code pending
                                XDSConnectWSSoapClient xdsClient =
                                    new XDSConnectWSSoapClient
                                    (XDSConnectWSSoapClient.EndpointConfiguration.XDSConnectWSSoap);

                                var result = xdsClient.ConnectLogResultAsync(
                                                "UserName", "Password", CustSubscriberId, XDSUsername,
                                                 profile.idNumber, Passport, firstName, surName, BirthDate,
                                                DateTime.Now.ToString(), InspiritRefNum, EnquiryInput,
                                                XDSRefNum, XDSVoucherCode
                                                );
                        ...
                        else
                        {
                            ...
                            //profile.SearchCriteria = request.SearchCriteria;
                            //profile.RefNum = tlogs.Result.RefNum.ToString();
                        }
        ...
        private PersonProfile GetProfileData(List<LookupData> lookupLst, bool istrail, DataSet dataSet, SqlConnection con, ProfileRequest request)
        {
                ...
                if (Normal == false)
                {
                    ...
                    if (dtCommercialDirectors.Rows.Count > 0)
                        foreach (DataRow drCommercialDirector in dtCommercialDirectors.Rows)
                        {
                            ...
                            // krishna  start
                            if (drCommercialDirector["CommercialTelephoneNo"] != DBNull.Value)
                                director.CommercialTelephoneNo = Convert.ToString(drCommercialDirector["CommercialTelephoneNo"]);
                            // krishna end
                        ...
                        foreach (DataRow dr in dtBuyer.Rows)
                        {
                            ...
                            foreach (DataRow drBuyer in dtBuyerExt.Rows)
                            {
                                if (drBuyer["PropertyDeedId"].ToString() == dr["PropertyDeedId"].ToString())
                                {
                                    ...
                                    property1.sellerIDNO = drBuyer["SellerIDNO"].ToString();
                                    property1.sellerName = drBuyer["SellerName"].ToString();
                                    break;
                                }
                            }
                    ...
                    DataTable dtSellerExt = dataSet.Tables[23];
                    ...
                    if (dtSellar.Rows.Count > 0)
                        ...
                        {
                            var property1 = new PropertyDeedDetail();
                            foreach (DataRow drSeller in dtSellerExt.Rows)
                            {

                                if (drSeller["PropertyDeedId"].ToString() == dr1["PropertyDeedId"].ToString())
                                {
                                    // string buyerid = property1.buyerid + " - " + drSeller["BuyerIDNO"].ToString();


                                    //BuyerIDNO, BuyerName              
                                    //property1.buyerIDNO = property1.buyerid + " - " + drSeller["BuyerIDNO"].ToString();
                                    //property1.buyerName = property1.buyerName + " - " + drSeller["BuyerName"].ToString();
                                    property1.buyerIDNO = drSeller["BuyerIDNO"].ToString();
                                    property1.buyerName = drSeller["BuyerName"].ToString();
                                    break;
                    ...
                    // krishna start pending AKA

                    DataTable dtAKA = dataSet.Tables[24];
                    if (dtAKA.Rows.Count > 0)
                    {
                        foreach (DataRow dr in dtAKA.Rows)
                        {
                            var aka = new AKADetail();
                            if (dr["LastUpdatedDate"] != DBNull.Value)
                                aka.lastUpdatedDate = Convert.ToDateTime(dr["LastUpdatedDate"]);
                            if (dr["ConsumerId"] != DBNull.Value)
                                aka.consumerId = Convert.ToString(dr["ConsumerId"]);
                            if (dr["AKAS"] != DBNull.Value)
                                aka.akas = Convert.ToString(dr["AKAS"]);
                    ...
                    //CustomerLog customerLog = _IDAScontext.CustomerLogs.OrderByDescending(x => x.RefNum).Where(t => (t.CompanyId == request.customerId) && (t.CompanyUserId == request.userId) && (t.IdOrPassportNumber == request.Id.ToString()) && ((t.DateTime.Year == day.Year) && (t.DateTime.Month == day.Month) && (t.DateTime.Day == day.Day)) && t.CreditPoints == 1).FirstOrDefault();
                    CustomerLog customerLog = _IDAScontext.CustomerLogs.OrderByDescending(x => x.RefNum).Where(t => (t.CompanyId == request.customerId) && (t.CompanyUserId == request.userId) && (t.SearchCriteria == request.SearchCriteria.ToString()) && ((t.DateTime.Year == day.Year) && (t.DateTime.Month == day.Month) && (t.DateTime.Day == day.Day)) && t.CreditPoints == 1).FirstOrDefault();
                    Customer cust = _IDAScontext.Customers.Where(c => c.Id == request.customerId).FirstOrDefault();
                    CustomerUser user = _IDAScontext.CustomerUsers.Where(c => c.Id == request.userId).FirstOrDefault();

                    pres.userName = user.Email;
                    pres.CompanyName = cust.RegistrationName;
                    // verify pending
                    pres.SearchCriteria = request.SearchCriteria;
                    //pres.EnquiryInputIdNo = request.SpouseID;
                    pres.RefNum = customerLog.RefNum.ToString();
                    pres.CustomerRefNum = customerLog.CustomerRefNum;
                    pres.SearchDateTime = DateTime.Now;

                    // krishna end pending AKA
                }
                else
                {
                    ...
                    if (dtAKA.Rows.Count > 0)
                    {
                        foreach (DataRow dr in dtAKA.Rows)
                        {
                            var aka = new AKADetail();
                            if (dr["LastUpdatedDate"] != DBNull.Value)
                                aka.lastUpdatedDate = DateTime.MinValue;
                            if (dr["ConsumerId"] != DBNull.Value)
                                aka.consumerId = Convert.ToString(dr["ConsumerId"]);
                            if (dr["AKAS"] != DBNull.Value)
                                aka.akas = Convert.ToString(dr["AKAS"]);
                    ...
                    CustomerLog customerLog = _IDAScontext.CustomerLogs.OrderByDescending(x => x.RefNum).Where(t => (t.CompanyId == request.customerId) && (t.CompanyUserId == request.userId) && (t.IdOrPassportNumber == request.Id.ToString()) && (t.DateTime < filterday && t.DateTime == day) && t.CreditPoints == 1).FirstOrDefault();
                    Customer cust = _IDAScontext.Customers.Where(c => c.Id == request.customerId).FirstOrDefault();
                    CustomerUser user = _IDAScontext.CustomerUsers.Where(c => c.Id == request.userId).FirstOrDefault();
